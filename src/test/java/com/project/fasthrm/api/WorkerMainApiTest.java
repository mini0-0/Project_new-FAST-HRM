...
public class WorkerMainApiTest {
    ...
    void givenPlaceId_whenRequestingLessons_thenReturnLessonList() throws Exception {
        ...
        List<EduDto> lessons = List.of(
                new EduDto("Math", "월수금", LocalDateTime.now(), LocalDateTime.now().plusHours(1), BigDecimal.valueOf(10000), false, "101호"),
                new EduDto("English", "화목", LocalDateTime.now(), LocalDateTime.now().plusHours(1), BigDecimal.valueOf(15000), false, "102호")
        );
    ...
    void givenWorkerId_whenRequestingLessons_thenReturnList() throws Exception {
        ...
        given(workerMainService.getEduListWorkerId(workerId)).willReturn(
                List.of(new EduDto("Science", "매일", LocalDateTime.now(), LocalDateTime.now().plusHours(1), BigDecimal.valueOf(10000), false, "201호"))
        );
    ...
    void givenPlaceId_whenRequestingTodayLessons_thenReturnTodayList() throws Exception {
        ...
        given(workerMainService.getTodayLessons(placeId)).willReturn(
                List.of(
                        new EduDto("Math", "월수금", LocalDateTime.now(), LocalDateTime.now().plusHours(1), BigDecimal.valueOf(12000), false, "101호"),
                        new EduDto("Science", "매일", LocalDateTime.now(), LocalDateTime.now().plusHours(1), BigDecimal.valueOf(13000), false, "201호")
                )
        );
    ...
    void givenMasterLessonRequest_whenPosting_thenCreateLesson() throws Exception {
        // given
        EduDto dto = new EduDto("NewLesson", "월수금", LocalDateTime.now(), LocalDateTime.now().plusHours(1), BigDecimal.valueOf(15000), false, "303호");
    ...
    void givenWorkerWithTuition_whenPosting_thenForbidden() throws Exception {
        // given
        EduDto dto = new EduDto("WorkerLesson", "화목", LocalDateTime.now(), LocalDateTime.now().plusHours(1), BigDecimal.valueOf(30000), false, "304호");
    ...
    void givenApprovedWorker_whenPosting_thenAllowed() throws Exception {
        // given
        EduDto dto = new EduDto("ApprovedLesson", "화목", LocalDateTime.now(), LocalDateTime.now().plusHours(1), BigDecimal.valueOf(30000), true, "505호");
    ...
    void givenMasterUpdateRequest_whenPut_thenUpdateLesson() throws Exception {
        // given
        EduDto dto = new EduDto("UpdatedLesson", "매일", LocalDateTime.now(), LocalDateTime.now().plusHours(1), BigDecimal.valueOf(18000), false, "606호");
